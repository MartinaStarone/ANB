...
/* AES-128: loop esterno i < 4, loop interno j < 4. Top-level: 4 iterazioni. */
__attribute__((annotate("trip_count_4")))
...
/* AES-128: doppio loop 4x4. Loop top-level: 4 iterazioni. */
__attribute__((annotate("trip_count_4")))
...
/* AES-128: loop i < 4. Top-level: 4 iterazioni. */
__attribute__((annotate("trip_count_4")))
...
/* AES-128: Nr=10 round. Il loop esegue da round=1 a round==Nr.
 * Tuttavia, al 10° round fa un break anticipato, quindi il latch del loop
 * viene eseguito solo 9 volte! ANB conta i latch, quindi annotiamo 9. */
__attribute__((annotate("trip_count_9")))
